...
const FOLDERS_TO_IGNORE = ['.git', 'node_modules', 'vendors', 'images', 'assets', 'public', 'dev', 'src_total'];
...
function main() {
    ...
    if (!fs.existsSync(projectRoot)) {
        console.error(`Error: Source folder not found at: ${projectRoot}`);
        return;
    }
    ...
    console.log(`Copying files from ${projectRoot} to ${destDirPath}`);

    const ig = readGitignore(projectRoot); // Gitignore from project root
    copyFilesToDestination(projectRoot, destDirPath, projectRoot, ig);
