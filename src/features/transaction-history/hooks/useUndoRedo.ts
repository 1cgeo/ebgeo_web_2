// Path: features/transaction-history/hooks/useUndoRedo.ts
...
export const useUndoRedo = (): UseUndoRedoReturn => {
  ...
  // Criar transação
  const createTransaction = useCallback((options: CreateTransactionOptions): string => {
    ...
    switch (type) {
      ...
      case 'update':
        if (!normalizedBefore || !normalizedAfter || 
          ...
          throw new Error('Transação de atualização deve ter dados "before" e "after" com mesmo tamanho');
        }
        break;
      ...
      case 'batch':
        if ((!normalizedBefore && !normalizedAfter) || 
            (normalizedBefore && normalizedAfter && normalizedBefore.length !== normalizedAfter.length)) {
  ...
  const applyChangesToStorage = useCallback(async (
  ...
  const revertTransaction = useCallback(async (transaction: Transaction): Promise<void> => {
  ...
  const applyTransaction = useCallback(async (transaction: Transaction): Promise<void> => {
    const { type, data } = transaction;
  ...
  // Retorno memorizado
  return useMemo((): UseUndoRedoReturn => ({
    // Estados
