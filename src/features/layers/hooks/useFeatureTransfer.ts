...
export const useFeatureTransfer = () => {
  ...
  const openTransferDialog = useCallback((featureIds?: string[]) => {
  ...
  const transferSelectedFeatures = useCallback(async (targetLayerId: string): Promise<FeatureTransferResult> => {
  ...
  const transferFeatures = useCallback(async (config: FeatureTransferConfig): Promise<FeatureTransferResult> => {
  ...
  const getTransferStats = useCallback((featureIds: string[], targetLayerId: string): FeatureTransferStats => {
    const features = selectedFeatures.filter(f => featureIds.includes(f.id));
    const targetLayer = availableLayers.find(l => l.id === targetLayerId);

    // Estatísticas por tipo de geometria
    const byGeometryType = features.reduce((acc, feature) => {
    ...
    const bySourceLayer = features.reduce((acc, feature) => {
  ...
  const validateTransfer = useCallback(async (featureIds: string[], targetLayerId: string) => {
    ...
    try {
      ...
      if (stats.conflictingFeatures.length > 0) {
        errors.push(`${stats.conflictingFeatures.length} feature(s) já estão na camada de destino`);
      }
  ...
  const transferLayerFeatures = useCallback(async (sourceLayerId: string, targetLayerId: string): Promise<FeatureTransferResult> => {
