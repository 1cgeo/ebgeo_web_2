...
import {
  ...
  FeatureFilters,
  FeatureSortOptions,
} from '../interfaces/IFeatureRepository';
...
export class IndexedDBFeatureRepository implements IFeatureRepository {
  ...
  async getByLayerId(layerId: string): Promise<ExtendedFeature[]> {
    try {
      ...
      throw new Error(
        `Falha ao buscar por camada: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async getByLayerIds(layerIds: string[]): Promise<ExtendedFeature[]> {
    try {
      ...
      throw new Error(
        `Falha ao buscar por camadas: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async getByBoundingBox(
    ...
    try {
      ...
      throw new Error(
        `Falha na busca espacial: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async getByRadius(
    ...
    try {
      ...
      throw new Error(
        `Falha na busca por proximidade: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async getByGeometryType(geometryType: string): Promise<ExtendedFeature[]> {
    try {
      return await db.features.filter(feature => feature.geometry.type === geometryType).toArray();
    } catch (error) {
      console.error('Erro ao buscar features por tipo de geometria:', error);
      throw new Error(
        `Falha ao buscar por geometria: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async moveToLayer(featureIds: string[], targetLayerId: string): Promise<ExtendedFeature[]> {
    try {
      ...
      throw new Error(
        `Falha ao mover para camada: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async duplicate(featureIds: string[], targetLayerId?: string): Promise<ExtendedFeature[]> {
    try {
      const originalFeatures = await Promise.all(featureIds.map(id => this.getById(id)));
      ...
      throw new Error(
        `Falha ao duplicar features: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
  ...
  async validateFeature(feature: ExtendedFeature): Promise<{
    ...
    try {
      ...
      errors.push(
        `Erro de validação: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  ...
  async getOrphanedFeatures(): Promise<ExtendedFeature[]> {
    try {
      ...
      return features.filter(feature => !validLayerIds.has(feature.properties.layerId));
    } catch (error) {
      console.error('Erro ao buscar features órfãs:', error);
      throw new Error(
        `Falha ao buscar órfãs: ${error instanceof Error ? error.message : 'Erro desconhecido'}`
      );
    }
  }
