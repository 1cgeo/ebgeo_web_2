...
export const useMapInstance = () => {
  ...
  const updateColdFeatures = useCallback((features: ExtendedFeature[]) => {
  ...
  const updateHotFeatures = useCallback((features: ExtendedFeature[]) => {
    if (!map || !isMapLoaded) return;
  ...
  const updateSelectedFeatures = useCallback((features: ExtendedFeature[]) => {
    if (!map || !isMapLoaded) return;
  ...
  const unproject = useCallback((point: maplibregl.Point): maplibregl.LngLat | null => {
    if (!map || !isMapLoaded) return null;
    return map.unproject(point);
  }, [map, isMapLoaded]);

  // Converter coordenadas geográficas para tela
  const project = useCallback((lngLat: maplibregl.LngLatLike): maplibregl.Point | null => {
    if (!map || !isMapLoaded) return null;
  ...
  const isPointVisible = useCallback((lngLat: maplibregl.LngLatLike): boolean => {
    if (!map || !isMapLoaded) return false;
    const bounds = map.getBounds();
    const point = typeof lngLat === 'object' && 'lng' in lngLat 
      ? lngLat 
  ...
  const animateFeature = useCallback((featureId: string, duration: number = 1000) => {
    if (!map || !isMapLoaded) return;
...
export const useMapDrawing = () => {
  ...
  const addTemporaryPoint = useCallback((lngLat: maplibregl.LngLatLike) => {
